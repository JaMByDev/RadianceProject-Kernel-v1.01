...
void OV5647MIPI_otp_read_enable(void)
{
...
void OV5647MIPI_otp_read_disable(void)
{
	OV5647MIPI_write_cmos_sensor(OTP_LOAD_ADDR, 0x00);
}

void OV5647MIPI_otp_read(unsigned short otp_addr, unsigned char* otp_data)
{
	OV5647MIPI_otp_read_enable();
	*otp_data = OV5647MIPI_read_cmos_sensor(otp_addr);
	OV5647MIPI_otp_read_disable();
}
...
* Return      :  none
*******************************************************************************/	
void OV5647MIPI_otp_clear(void)
{
...
* Return      :  0, group index is empty
                ...
                -1, group index error
*******************************************************************************/	
signed char OV5647MIPI_otp_check_wb_group(unsigned char index)
{   
	...
	OV5647MIPI_otp_read(otp_addr, &flag);
	OV5647MIPI_otp_clear();
...
* Return      :  group index (0, 1, 2)
                 -1, error
*******************************************************************************/	
signed char OV5647MIPI_otp_read_wb_group(signed char index)
{
	...
	if (index == -1)
	{
		...
		{
			if (OV5647MIPI_otp_check_wb_group(index) == 2)
			{
	...
	OV5647MIPI_otp_read(otp_addr, &mid);
	TRACE("[TINNO] mingji test read MID=%x\n",(mid&0x7f));
	if ((mid&0x7f) != Truly_OTP_MID)
	{
		return -1;
	}

	OV5647MIPI_otp_read(otp_addr+2, &rg_ratio);
	OV5647MIPI_otp_read(otp_addr+3, &bg_ratio);
	OV5647MIPI_otp_clear();
...
*******************************************************************************/	
bool OV5647MIPI_otp_apply_wb(unsigned char golden_rg, unsigned char golden_bg)
{
...
* Return      :  1, success; 0, fail
*******************************************************************************/	
bool OV5647MIPI_otp_update_wb(unsigned char golden_rg, unsigned char golden_bg) 
//bool otp_update_wb(void)
{
	TRACE("[TINNO] start wb update\n");

	if (OV5647MIPI_otp_read_wb_group(-1) != -1)
	{
		if (OV5647MIPI_otp_apply_wb(golden_rg, golden_bg) == 1)
		{
