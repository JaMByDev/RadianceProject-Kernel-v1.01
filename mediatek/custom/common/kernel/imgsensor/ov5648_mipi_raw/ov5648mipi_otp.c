...
unsigned char OV5648_RG_MSB = 0;
unsigned char OV5648_BG_MSB = 0;
unsigned char OV5648_AWB_LSB = 0;


// Enable OTP read function
void OV5648_otp_read_enable(void)
{
	OV5648MIPI_write_cmos_sensor(OTP_LOAD_ADDR, 0x01);
	mdelay(15); // sleep > 10ms
}

// Disable OTP read function
void OV5648_otp_read_disable(void)
{
	OV5648MIPI_write_cmos_sensor(OTP_LOAD_ADDR, 0x00);
}

void OV5648_otp_read(unsigned short otp_addr, unsigned char* otp_data)
{
	OV5648_otp_read_enable();
	*otp_data = OV5648MIPI_read_cmos_sensor(otp_addr);
	OV5648_otp_read_disable();
}
...
* Return      :  0, group index is empty
                ...
                -1, group index error
*******************************************************************************/	
signed char OV5648_otp_check_wb_group(unsigned char index)
{   
    	...
    	if (index > 2)
	{
   	...
   	{
   		// select bank 0
   		OV5648MIPI_write_cmos_sensor(OTP_BANK_ADDR, 0xc0);
   		OV5648MIPI_write_cmos_sensor(OTP_BANK, 0x00);
   		OV5648MIPI_write_cmos_sensor(OTP_END_ADDR, 0x0f);	
   	}
   	else /*index==2*/
   	{
   		// select bank 1
   		OV5648MIPI_write_cmos_sensor(OTP_BANK_ADDR, 0xc0);
   		OV5648MIPI_write_cmos_sensor(OTP_BANK, 0x10);
   		OV5648MIPI_write_cmos_sensor(OTP_END_ADDR, 0x1f);	
   		otp_addr = 0x3D07;
   	}
    
	OV5648_otp_read(otp_addr, &flag);
	OV5648_otp_clear();
...
* Return      :  group index (0, 1, 2)
                 -1, error
*******************************************************************************/	
signed char OV5648_otp_read_wb_group(signed char index)
{
	...
	if (index == -1)
	{
		...
		{
			if (OV5648_otp_check_wb_group(index) == 2)
			{
	...
	if (index == 0)
	{
		// select bank 0
   		...
   		OV5648MIPI_write_cmos_sensor(OTP_BANK, 0x00);
   		OV5648MIPI_write_cmos_sensor(OTP_END_ADDR, 0x0f);	
   		OV5648_otp_read(OTP_WB_GROUP_ADDR+2, &OV5648_RG_MSB);
		OV5648_otp_read(OTP_WB_GROUP_ADDR+3, &OV5648_BG_MSB);
		OV5648_otp_read(OTP_WB_GROUP_ADDR+6, &OV5648_AWB_LSB);
	}
	else /* (index == 1 || index ==2)*/
	{
		...
		// select bank 1
   		...
   		OV5648MIPI_write_cmos_sensor(OTP_BANK, 0x10);
   		OV5648MIPI_write_cmos_sensor(OTP_END_ADDR, 0x1f);	
   		OV5648_otp_read(awb_addr, &OV5648_RG_MSB);
		OV5648_otp_read(awb_addr+1, &OV5648_BG_MSB);
		OV5648_otp_read(awb_addr+4, &OV5648_AWB_LSB);	
	}

/*	mid_addr = OTP_WB_GROUP_ADDR + index * OTP_WB_GROUP_SIZE;
	OV5648_otp_read(mid_addr, &mid);
	printk("[OV5648_otp_read_wb_group],read MID=%x\n",(mid&0x7f));
	if ((mid&0x7f) != OTP_MID)
	{
		return -1;
	}
...
*******************************************************************************/	
bool OV5648_otp_apply_wb(unsigned short golden_rg, unsigned short golden_bg)
{
	unsigned short gain_r = GAIN_DEFAULT_VALUE;
	unsigned short gain_g = GAIN_DEFAULT_VALUE;
	unsigned short gain_b = GAIN_DEFAULT_VALUE;
    	unsigned short rg_ratio = (OV5648_RG_MSB<<2) | ((OV5648_AWB_LSB & 0xC0)>>6);
    	unsigned short bg_ratio = (OV5648_BG_MSB<<2) | ((OV5648_AWB_LSB & 0x30)>>4);
	...
	if (ratio_r != 1)
	{
		...
		OV5648MIPI_write_cmos_sensor(GAIN_RH_ADDR, gain_r >> 8);
		OV5648MIPI_write_cmos_sensor(GAIN_RL_ADDR, gain_r & 0x00ff);
	}

	if (ratio_g != 1)
	{
		gain_g = (unsigned short)(GAIN_DEFAULT_VALUE * ratio_g);
		OV5648MIPI_write_cmos_sensor(GAIN_GH_ADDR, gain_g >> 8);
		OV5648MIPI_write_cmos_sensor(GAIN_GL_ADDR, gain_g & 0x00ff);
	}

	if (ratio_b != 1)
	{
		gain_b = (unsigned short)(GAIN_DEFAULT_VALUE * ratio_b);
		OV5648MIPI_write_cmos_sensor(GAIN_BH_ADDR, gain_b >> 8);
		OV5648MIPI_write_cmos_sensor(GAIN_BL_ADDR, gain_b & 0x00ff);
	}
...
bool OV5648_otp_apply_wb(unsigned char golden_rg, unsigned char golden_bg)
{
	unsigned short gain_r = GAIN_DEFAULT_VALUE;
	unsigned short gain_g = GAIN_DEFAULT_VALUE;
	unsigned short gain_b = GAIN_DEFAULT_VALUE;
...
* Return      :  1, success; 0, fail
*******************************************************************************/	
bool OV5648_otp_update_wb(unsigned short golden_rg, unsigned short golden_bg) 
{
	printk("start wb update\n");

	if (OV5648_otp_read_wb_group(-1) != -1)
	{
		if (OV5648_otp_apply_wb(golden_rg, golden_bg) == 1)
		{
