...
#include "ov5645mipi_Sensor.h"
#include "ov5645mipi_Camera_Sensor_para.h"
#include "ov5645mipi_CameraCustomized.h"


extern kal_uint16 OV5645MIPI_write_cmos_sensor(kal_uint32 addr, kal_uint32 para);
extern kal_uint16 OV5645MIPI_read_cmos_sensor(kal_uint32 addr);
...
unsigned char OV5645MIPI_rg_ratio = 0;
unsigned char OV5645MIPI_bg_ratio = 0;


// Enable OTP read function
void OV5645MIPI_otp_read_enable(void)
{
	OV5645MIPI_write_cmos_sensor(OTP_LOAD_ADDR, 0x01);
	mdelay(10); // sleep > 10ms
}

// Disable OTP read function
void OV5645MIPI_otp_read_disable(void)
{
	OV5645MIPI_write_cmos_sensor(OTP_LOAD_ADDR, 0x00);
}

void OV5645MIPI_otp_read(unsigned short otp_addr, unsigned char* otp_data)
{
	OV5645MIPI_otp_read_enable();
	*otp_data = OV5645MIPI_read_cmos_sensor(otp_addr);
	OV5645MIPI_otp_read_disable();
}
...
* Return      :  0, group index is empty
                ...
                -1, group index error
*******************************************************************************/	
signed char OV5645MIPI_otp_check_wb_group(unsigned char index)
{   
	...
	OV5645MIPI_otp_read(otp_addr, &flag);
	OV5645MIPI_otp_clear();
...
* Return      :  group index (0, 1, 2)
                 -1, error
*******************************************************************************/	
signed char OV5645MIPI_otp_read_wb_group(signed char index)
{
	...
	if (index == -1)
	{
		...
		{
			if (OV5645MIPI_otp_check_wb_group(index) == 2)
			{
	...
	OV5645MIPI_otp_read(otp_addr, &mid);
	TRACE("[TINNO] mingji test read MID=%x\n",(mid&0x7f));
	if (((mid&0x7f) != Darling_OTP_MID)&&((mid&0x7f) != Darling_OTP_MID2))
	{
		return -1;
	}

	OV5645MIPI_otp_read(otp_addr+2, &OV5645MIPI_rg_ratio);
	OV5645MIPI_otp_read(otp_addr+3, &OV5645MIPI_bg_ratio);
	OV5645MIPI_otp_clear();
...
*******************************************************************************/	
bool OV5645MIPI_otp_apply_wb(unsigned char golden_rg, unsigned char golden_bg)
{
	...
	cmp_rg = 1.0 * OV5645MIPI_rg_ratio / golden_rg;
	cmp_bg = 1.0 * OV5645MIPI_bg_ratio / golden_bg;
	...
	if (ratio_r != 1)
	{
		...
		OV5645MIPI_write_cmos_sensor(GAIN_RH_ADDR, gain_r >> 8);
		OV5645MIPI_write_cmos_sensor(GAIN_RL_ADDR, gain_r & 0x00ff);
	}

	if (ratio_g != 1)
	{
		gain_g = (unsigned short)(GAIN_DEFAULT_VALUE * ratio_g);
		OV5645MIPI_write_cmos_sensor(GAIN_GH_ADDR, gain_g >> 8);
		OV5645MIPI_write_cmos_sensor(GAIN_GL_ADDR, gain_g & 0x00ff);
	}

	if (ratio_b != 1)
	{
		gain_b = (unsigned short)(GAIN_DEFAULT_VALUE * ratio_b);
		OV5645MIPI_write_cmos_sensor(GAIN_BH_ADDR, gain_b >> 8);
		OV5645MIPI_write_cmos_sensor(GAIN_BL_ADDR, gain_b & 0x00ff);
	}
...
bool OV5645MIPI_otp_apply_wb(unsigned char golden_rg, unsigned char golden_bg)
{
	...
	// Calcualte R, G, B gain of current module from R/G, B/G of golden module
    // and R/G, B/G of current module
	cmp_rg = OTP_MULTIPLE_FAC * OV5645MIPI_rg_ratio / golden_rg;
	cmp_bg = OTP_MULTIPLE_FAC * OV5645MIPI_bg_ratio / golden_bg;
...
* Return      :  1, success; 0, fail
*******************************************************************************/	
bool OV5645MIPI_otp_update_wb(unsigned char golden_rg, unsigned char golden_bg) 
//bool otp_update_wb(void)
{
	TRACE("[TINNO] start wb update\n");

	if (OV5645MIPI_otp_read_wb_group(-1) != -1)
	{
		if (OV5645MIPI_otp_apply_wb(golden_rg, golden_bg) == 1)
		{
