...
kal_uint16 OV8825_read_customer_mid(void)
{
	...
	OV8825DB("OV8825_read_customer_mid()!!!!!\n");
	OV8825DB("OV8825_read_customer_mid()!!!!!\n");
	...
	for(index = 0; index < 3; index++)
	{
	    ...
	    OV8825DB("OV8825_read_customer_mid()!!!!! index==%d,mid ==%d\n", index,OV8825_read_cmos_sensor(address) & 0x7f);
	    OV8825DB("OV8825_read_customer_mid()!!!!! index==%d,lens_id ==%d\n", index,OV8825_read_cmos_sensor(address+1));
	    if(((0x01 == (OV8825_read_cmos_sensor(address) & 0x7f))&&(0x01 == OV8825_read_cmos_sensor(address+1)))
                    ||((0x02 == (OV8825_read_cmos_sensor(address) & 0x7f))&&(0x0e == OV8825_read_cmos_sensor(address+1)))){
	        OV8825DB("************* This is sunny TDK+Largen or truly CM7534[9566A1] module.\n");
		break;
	    }
	}	
...
static void OV8825_Sensor_Init(void)
{
	...
	OV8825_write_cmos_sensor(0x361c, 0x07); //VCM clock divider	
...
/*	OV8825_write_cmos_sensor(0x3d00, 0x00);
	...
	*/
	...
	//OV8825_write_cmos_sensor(0x4831, 0x06); //ui
	//; ISP
...
UINT32 OV8825GetSensorID(UINT32 *sensorID) 
{
    ...
    do {
        *sensorID = (OV8825_read_cmos_sensor(0x300A)<<8)|OV8825_read_cmos_sensor(0x300B);        
        ...
        {
	    if (2 == OV8825_read_customer_mid())
	    {
		OV8825DB("************* This is sunny TDK+Largen or truly CM7534[9566A1] module.\n");
	    }
	    else
	    {
        	OV8825DB("************* This is truly CM7625[K1JR] new module.\n");
        	*sensorID = 0xFFFFFFFF; 
        	return ERROR_SENSOR_CONNECT_FAIL;
	    }
...
UINT32 OV8825GetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
    ...
    OV8825DB("OV8825GetResolution!!\n");
		...
		{
        	case MSDK_SCENARIO_ID_CAMERA_ZSD:
				pSensorResolution->SensorPreviewWidth	= OV8825_IMAGE_SENSOR_FULL_WIDTH;
    			pSensorResolution->SensorPreviewHeight	= OV8825_IMAGE_SENSOR_FULL_HEIGHT;
				break;
			default:
...
{	
	//#if defined(MT6575)||defined(MT6577)
		...
		{
        	case MSDK_SCENARIO_ID_CAMERA_ZSD:
				pSensorInfo->SensorPreviewResolutionX= OV8825_IMAGE_SENSOR_FULL_WIDTH;
    			pSensorInfo->SensorPreviewResolutionY= OV8825_IMAGE_SENSOR_FULL_HEIGHT;
				break;
			default:
...
UINT32 OV8825SetVideoMode(UINT16 u2FrameRate)
{
	...
	else if(ov8825.sensorMode == SENSOR_MODE_CAPTURE)
	{
		...
		if(ov8825.OV8825AutoFlickerMode == KAL_TRUE)
    	{
			...
			#endif
			MAX_Frame_length = (ov8825.pvPclk*10000) /(OV8825_FULL_PERIOD_PIXEL_NUMS + ov8825.DummyPixels)/frameRate*10;
    	}
		else
		...
		OV8825DB("[OV8825SetVideoMode]current fps (10 base)= %d\n", (ov8825.pvPclk*10000)*10/(OV8825_FULL_PERIOD_PIXEL_NUMS + ov8825.DummyPixels)/MAX_Frame_length);
