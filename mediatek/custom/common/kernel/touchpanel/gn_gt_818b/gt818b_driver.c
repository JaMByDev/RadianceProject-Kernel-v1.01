...
#ifdef MT6575
#include "tpd_custom_gn_gt_818b.h"
...
extern s32 gt818x_downloader(struct i2c_client *client);
...
#define I2C_MASTER_CLOCK       300       //400 发现在中断中IIC资源会申请不成功
...
reset_proc:   
    ...
    else
    {
        ...
        {
            TPD_DMESG("GT818X update.\n");
            gt818x_downloader(client);
        }

#endif
        
#if 0 //AUTOUPDATE_FIRMWARE
	...
	u8 module_no = 0;
	err = i2c_read_bytes( client, 0x710, (u8 *)&module_no, sizeof(module_no) );
	printk("CHIVA TP module_no = %d, return value = %d\n", module_no, err);
	/*
	#if (defined(GN_MTK_BSP_TP_SUPPORT_OFILM) || defined(GN_MTK_BSP_TP_SUPPORT_TRULY) )
	#if (defined(GN_MTK_BSP_TP_SUPPORT_OFILM)
	err = 0;
	module_no = 0;
	#endif
	#if defined(GN_MTK_BSP_TP_SUPPORT_TRULY)
	err = 0;
	module_no = 1;
	#endif
	printk("CHIVA TP module_no = %d, return value = %d\n", module_no, err);
	*/
	if (err == 0 && module_no == 0)
	{
		cfg_data_version_b		 			= cfg_data_version_b_ofilm;
		cfg_data_with_charger_version_b 	= cfg_data_with_charger_version_b_ofilm;
		cfg_data_version_d 					= cfg_data_version_d_ofilm;
		cfg_data_with_charger_version_d 	= cfg_data_with_charger_version_d_ofilm;
	}
	else if(err == 0 && module_no == 2)
	{
		cfg_data_version_b		 			= cfg_data_version_b_truly;
		cfg_data_with_charger_version_b 	= cfg_data_with_charger_version_b_truly;
		cfg_data_version_d 					= cfg_data_version_d_truly;
		cfg_data_with_charger_version_d 	= cfg_data_with_charger_version_d_truly;
	}
	else
	{
		cfg_data_version_b		 			= cfg_data_version_b_wintek;
		cfg_data_with_charger_version_b 	= cfg_data_with_charger_version_b_wintek;
		cfg_data_version_d 					= cfg_data_version_d_wintek;
		cfg_data_with_charger_version_d 	= cfg_data_with_charger_version_d_wintek;
	}
	#endif
	//\\
...
#if 0	
		if((wrap_x >= 1)&&(wrap_x < 20))
		{
			wrap_x = wrap_x + 4;
		}
		else if((wrap_x>=20)&&(wrap_x < 38))
		{
			 wrap_x =  wrap_x + 4;
		}
		else if((wrap_x>435)&&( wrap_x < 460))
		{
			wrap_x = wrap_x - 4;

		}
		else if((wrap_x >= 460)&&( wrap_x < 480))
		{
			 wrap_x -= 4;
		}
		if((wrap_y >= 1)&&( wrap_y <= 35))
		{
			 wrap_y = wrap_y + 8;
		}
		else if(( wrap_y < 60)&&(wrap_y > 35))
		{
			 wrap_y =  wrap_y + 6;
		}
		
		if(( wrap_y > 745)&&( wrap_y <= 775))
		{
			 wrap_y =  wrap_y - 2;
		}
		else if(( wrap_y < 800)&&( wrap_y > 775))
		{
			wrap_y -= 2;
		}
#endif						
		...
		if ( tpd != NULL && tpd->dev != NULL )
	        ...
	        else
	        {
		     for(count = 0; count < 4; count++)
		     {
			  ...
			  if( key&(0x01<<count) ) //key=1 menu ;key=2 home; key =4 back; key=8 search
			  {
...
static void tpd_suspend( struct early_suspend *h )
{
    ...
    tpd_halt = 1;     
#ifdef ESD_PROTECT
	cancel_delayed_work_sync(tpd_esd_check_workqueue);
#endif
