...
static struct i2c_board_info __initdata kd_lens_dev={ I2C_BOARD_INFO("OV8835AF", 0x18)};
...
static int OV8835AF_Release(struct inode * a_pstInode, struct file * a_pstFile)
{
    if (g_s4OV8835AF_Opened)
    {
        OV8835AFDB("[OV8835AF] free \n");

	    s4OV8835AF_WriteReg(200);
        msleep(10);
	    s4OV8835AF_WriteReg(100);
        msleep(10);
	    s4OV8835AF_WriteReg(50);
        msleep(10);
	    s4OV8835AF_WriteReg(20);
        msleep(10);
            s4OV8835AF_WriteReg(0);
...
static int OV8835AF_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    OV8835AFDB("[OV8835AF] Attach I2C \n");
